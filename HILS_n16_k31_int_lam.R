...
start_design_list_Var_s= replicate(25, matrix(sample(c(-1,1), n*p, TRUE), nrow = n, ncol = p))
#Generating Var(s+) designs may take some time given Byran's proposed changes have been implemented
Var_s_values = rep(NA, 25)
...
submodel_index = sample(c(1:(choose(31,6))), size=20, replace = FALSE)

all_submodels= gen_all_submodels(31,k_star)

A_val= all_submodels[submodel_index,]
